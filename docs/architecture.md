...
- daemon.ts 依赖 B/C/D/E/G（编排所有自动化循环）
...
│  ┌──────────────────────────────────────────────┐  │
│  │           事件记录层（完全确定性）              │  │
│  │  检测 .md 变更 → 对比 claims → 记录 EditEvent  │  │
│  │  存储: _meta/edit_events.yaml                  │  │
│  └──────────────────┬───────────────────────────┘  │
│                     │                               │
│                     ▼                               │
│  ┌──────────────────────────────────────────────┐  │
...
│  │  周期性分析积累事件 → 提取行为模式              │  │
│  │  - 按 InfoType 统计编辑频率                    │  │
│  │  - 检测 false_negative（fresh 被改 claims）    │  │
│  │  - 检测 skip_then_edit（stale 被手动更新）     │  │
│  └──────────────────┬───────────────────────────┘  │
│                     │                               │
│                     ▼                               │
│  ┌──────────────────────────────────────────────┐  │
│  │         阈值校准层（确定性公式）               │  │
│  │  只有积累足够信号才触发校准：                   │  │
│  │  - 3+ 次同类型 false_negative → ×0.7          │  │
│  │  - 2+ 次同类型 skip_then_edit → ×0.85         │  │
│  │  避免单次偶然编辑导致半衰期大幅波动             │  │
│  └──────────────────────────────────────────────┘  │
│                                                     │
├─ 非确定性内核（受控层）──────────────────────────────┤
│                                                     │
│  ┌─────────────┐  ┌─────────────────────────────┐  │
│  │ LLM Ingest  │  │ LLM Freshness Verification  │  │
│  │ (自动摄入)   │  │ (自动保鲜检查)               │  │
│  └─────────────┘  └─────────────────────────────┘  │
│                                                     │
│  降级策略：LLM 不可用时静默跳过，不中断确定性循环     │
└─────────────────────────────────────────────────────┘
```
...
| 周期 | 操作 | 确定性？ | 降级策略 |
|------|------|---------|---------|
| 每 6h | crawl → 有 API Key 时自动 ingest | 抓取确定性，ingest 非确定性 | 无 Key 时只抓取不 ingest |
| 每 24h | freshcheck → 生成报告 | 扫描确定性，验证非确定性 | 无 Key 时跳过 |
| 每 168h | Pattern 分析 → 阈值校准 | 完全确定性 | 无降级需要 |
| 实时 | fs.watch 监听 → 记录 EditEvent | 完全确定性 | 无降级需要 |

### 关键设计原则

1. **不对单次编辑立即反应**：改了一个页面不代表半衰期就该变
2. **积累信号再分析**：多次同类行为才构成 pattern
3. **确定性优先**：调度、统计、公式全部确定性，LLM 只在受控位置使用
4. **优雅降级**：LLM 不可用时系统不崩溃，退化为纯确定性运行
